    ...
    */
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr) && keywords.vt == (VT_LPWSTR|VT_VECTOR)) {
            ...
            for (ULONG i = 0; i < numKeywords; i++) {
                const char* sqlmsg = R"(INSERT OR IGNORE INTO tags(file_id, tag_name) VALUES (?, ?))";
                sqlite3_stmt* stmt;

                int rc = sqlite3_prepare_v2(&db, sqlmsg, -1, &stmt, nullptr);
                if (rc != SQLITE_OK) {
                    cerr << "Failed to prepare statement 4: " << sqlite3_errmsg(&db) << endl;
                    return;
                }
                ...
                wstring ws(keywords.calpwstr.pElems[i]);
                string tagName(ws.begin(), ws.end());
                sqlite3_bind_text(stmt, 2, tagName.c_str(), -1, SQLITE_STATIC);

                rc = sqlite3_step(stmt);
                if (rc != SQLITE_DONE) {
                    cerr << "Error executing statement 5: " << sqlite3_errmsg(&db) << endl;
                }
...
void getAndSaveDriveData(sqlite3& db) {
    ...
    if (GetVolumeInformation(NULL, volumeName, sizeof(volumeName), &serialNumber, &maxComponentLen, &fileSystemFlags, fileSystemName, sizeof(fileSystemName))) {
        ...
        const char* sqlmsg = R"(SELECT * FROM drives WHERE drive_id_win = ?)";
        sqlite3_stmt* stmt;
        ...
        // If the drive does not exist, create a new drive. At this point in time, we do not know the mac drive ID, so this will be left as null. 
        if (!exists) {
            sqlmsg = R"(INSERT OR IGNORE INTO drives(drive_id_win, drive_id_mac, drive_name) 
            VALUES (?, ?, ?))";
            ...
            sqlite3_bind_int(stmt, 1, serialNumber);
            sqlite3_bind_null(stmt, 2);
            sqlite3_bind_text(stmt, 3, volumeName, -1, SQLITE_STATIC);
...
void createFilesSQLTable(int fileId, filesystem::path filePath, sqlite3& db, DWORD volumeId) {
    const char* sqlmsg = R"(INSERT OR IGNORE INTO files(file_id_win, file_id_mac, drive_uuid, file_path) 
    VALUES (?, ?, ?, ?))";
    sqlite3_stmt* stmt;

    int rc = sqlite3_prepare_v2(&db, sqlmsg, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        cerr << "Failed to prepare statement 3: " << sqlite3_errmsg(&db) << endl;
        return;
    }
    ...
    sqlite3_bind_text(stmt, 4, filePath.filename().string().c_str(), -1, SQLITE_STATIC);
